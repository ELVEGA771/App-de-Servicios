...
SELECT 
    ...
    -- Ingresos Totales (Ganancia Empresa)
    (SELECT COALESCE(SUM(c.ganancia_empresa), 0)
     FROM contratacion c 
     JOIN servicio s ON c.id_servicio = s.id_servicio 
     WHERE s.id_empresa = e.id_empresa AND c.estado = 'completado') AS ingresos_totales,
    -- Subtotal (Total pagado por clientes)
    (SELECT COALESCE(SUM(c.precio_total), 0)
     FROM contratacion c 
     JOIN servicio s ON c.id_servicio = s.id_servicio 
     WHERE s.id_empresa = e.id_empresa AND c.estado = 'completado') AS subtotal_ventas,
    -- Comision App
    (SELECT COALESCE(SUM(c.comision_plataforma), 0)
     FROM contratacion c 
     JOIN servicio s ON c.id_servicio = s.id_servicio 
     WHERE s.id_empresa = e.id_empresa AND c.estado = 'completado') AS comision_app,
